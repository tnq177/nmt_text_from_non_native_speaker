...
class Trainer(object):
    ...
    def train(self):
        ...
        for e in range(self.max_epochs):
            b = 0
            for batch_data in self.data_manager.get_batch(ids_file=train_ids_file, shuffle=True, num_preload=self.num_preload):
                b += 1
        ...
        for checkpoint in self.data_manager.checkpoints:
            ...
            dev_file = self.data_manager.dev_files[checkpoint][self.data_manager.src_lang]
            test_file = self.data_manager.test_files[checkpoint][self.data_manager.src_lang]
            if exists(test_file):
    ...
    def maybe_validate(self, just_validate=False):
        if self.num_batches_done % self.validate_freq == 0 or just_validate:
            ...
            # if doing annealing
            if self.config['warmup_style'] == ac.NO_WARMUP and self.lr_decay > 0:
